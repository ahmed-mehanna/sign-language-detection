...
def extract_all_keypoints_video(words,video):
    ...
    for i,word in enumerate(words):
        ...
        for frame in video[i]:
            ...
            #     break
        new_output.append(current_vid)
    cv2.destroyAllWindows()
    return new_output

    
def display_keypoints_video(words,features):
    
    for i,word in enumerate(words):
        for results in features[i]:
            new_frame = np.zeros((960,1280,3)) + 255
            draw_styled_landmarks(new_frame, results)
            cv2.rectangle(new_frame, (0,0), (640, 40), (245, 117, 16), -1)
            write_text(new_frame," ".join(words[-5:]),(10,30))
            cv2.imshow("current frame",new_frame)
            if cv2.waitKey(25) & 0xFF == ord('q'):
                break
    cv2.destroyAllWindows()
...
features = extract_all_keypoints_video(words,video)
display_keypoints_video(words,features)
